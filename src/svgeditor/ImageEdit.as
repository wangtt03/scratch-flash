...
public class ImageEdit extends Sprite {
	...
	private function workAreaMouseDown(event:MouseEvent):void {
		...
		if (editable) {
			...
			if (CursorTool.tool == 'grow' || CursorTool.tool == 'shrink') {
				...
				var center:Point = obj.parent.globalToLocal(
						obj.localToGlobal(Point.interpolate(rect.topLeft, rect.bottomRight, 0.5)));
				...
				var ofs:Point = center.subtract(
						obj.parent.globalToLocal(
								obj.localToGlobal(Point.interpolate(rect.topLeft, rect.bottomRight, 0.5))));
				obj.x += ofs.x;
				obj.y += ofs.y;
	...
	protected function stageKeyDownHandler(event:KeyboardEvent):Boolean {
		...
		else if (toolMode != 'select' && currentTool is SVGEditTool &&
	...
	private function createTools():void {
		...
		for (var i:int = 0; i < tools.length; ++i) {
			if (tools[i] == null) dy += extraSpace;
				...
				ib = new IconButton(
						isImmediate ? handleImmediateTool : selectTool, makeToolButton(
								iconName, true, buttonSize), makeToolButton(
								iconName, false, buttonSize), !isImmediate);
	...
	public function setToolMode(newMode:String, bForce:Boolean = false, fromButton:Boolean = false):void {
		...
		switch (newMode) {
			case 'select':
				currentTool = new ObjectTransformer(this);
				break;
			case 'pathedit':
				currentTool = new PathEditTool(this);
				break;
			case 'path':
				currentTool = new PathTool(this);
				break;
			case 'vectorLine':
			case 'line':
				currentTool = new PathTool(this, true);
				break;
			case 'vectorEllipse':
			case 'ellipse':
			...
			case 'eraser':
				...
				break;
			case 'clone':
				currentTool = new CloneTool(this);
				break;
			case 'eyedropper':
				currentTool = new EyeDropperTool(this);
				break;
			case 'vpaintbrush':
				currentTool = new PaintBrushTool(this);
				break;
			case 'setCenter':
				currentTool = new SetCenterTool(this);
				break;
			// Add bitmap tools here....
			case 'bitmapBrush':
				currentTool = new BitmapPencilTool(this, false);
				break;
			case 'bitmapEraser':
				currentTool = new BitmapPencilTool(this, true);
				break;
			case 'bitmapSelect':
				currentTool = new ObjectTransformer(this);
				break;
	...
	protected function updateDrawPropsForTool(newMode:String):void {
		...
		var strokeModes:Array = [
			'bitmapBrush', 'line', 'rect', 'ellipse', 'select', 'pathedit', 'path', 'vectorLine', 'vectorRect',
			'vectorEllipse'
		];
